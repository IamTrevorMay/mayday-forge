...
struct MoveFileAction: ForgeAction {
    static let typeId = "move-file"
    static let displayName = "Move File"
    static let description = "Move a file to a destination folder (auto-creates destination, supports variables)"
    static let configFields: [ActionConfigField] = [
        ActionConfigField(key: "destination", label: "Destination Folder", placeholder: "/path/to/{date}", fieldType: .folderPicker),
    ...
    func execute(trigger: TriggerEvent) async throws -> ActionResult {
        ...
        return .ok(["Moved \(fileName) -> \(destPath)"])
    }
}
